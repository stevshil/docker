...
// Database Settings:
	...
	$nuConfigDBHost						= getenv("DBHOSTNAME");				//-- Database Host / IP. You may try localhost if 127.0.0.1 does not work.
	$nuConfigDBName						= getenv("DBNAME");				//-- Database Name. You can change the name, if desired. The database must exist or must be created on your server.

	$nuConfigDBUser						= getenv("DBUSER");					//-- Database User.  It is strongly recommended to use a different user than root The user must exist or must be created.
	$nuConfigDBPassword					= getenv("DBPASS");						//-- Database Password. We strongly recommend you to use any strong password.
...
// Administrator Login:
	...
	$nuConfigDBGlobeadminPassword		= getenv("GLOBALADMINPW");						//-- Administrator password. Please choose a stronger password!
